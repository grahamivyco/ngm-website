...
1. `01-top.slim.html` — Custom HTML (hero) — OPENS wrapper divs, does NOT close them
2. `02-wa-gadget.txt` — native WA month-grid "Events Calendar", class `ngm-cal`
3. `03-bottom.slim.html` — Custom HTML — CLOSES the wrapper divs, then the **Key** module + Join CTA

## Toolbar / weekday JS
`01-top.slim.html` (and `01-top.slim.html`) ship a small inline `<script>` the calendar
...
### Key / legend
`03-bottom.slim.html` carries a `.ngm-key` module whose swatches use the **exact**
